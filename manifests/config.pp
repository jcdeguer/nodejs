class nodejs::config inherits nodejs::params {
  ...
  file_line { 'paths':
        line => 'PATH=/node/.npm-global/bin:/usr/local/bin:/usr/local/sbin:~/bin/usr/local/share/npm/bin:$PATH',
        path => $bash_profile,
  }->
  ...
  file_line { 'node_paths':
        line => 'NODE_PATH=/node/.npm-global/lib/node_modules/:$NODE_PATH',
        path => $bash_profile,
  }->
...
# Instalacion del modulo oracledb
  exec { 'seteo_prefixs_en_npm':
        command => '/usr/bin/npm config set prefix "/node/.npm-global/"',
  }->
  exec { 'seteo_proxi_en_npm':
        command => '/usr/bin/npm config set proxy="http://IP_PROXY:PUERTO_PROXY"', # Donde IP_PROXY:PUERTO_PROXY corresponde a tu configuracion proxy
  }
  exec { 'install_oracldb_con_npm': # Este modulo valida si existe oracldb instalado, en caso contrario, lo instala.
        cwd => '/apps-node/',
        command => '/usr/bin/npm install -g oracledb --prefix /node/.npm-global/ -ve',
        user => 'node',
        creates => '/node/.npm-global/lib/node_modules/oracledb',
  }
}
